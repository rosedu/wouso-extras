...
public class ActiveChallenge extends Activity {
	...
	public void startChallenge(int challenge_id) {
		...
		final ActiveChallenge datJava = this;
		...
		next.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (activeChallenge == totalChallenges) {
					...
					finishActivity(1);
					Intent x = new Intent(datJava, ChallengeMenu.class);
					startActivity(x);
					return;
	...
	public Map<String, String> getWrappable() {
		...
		for(int i=0;i<=4;i++) {
			...
			String toBuild = "";
			for(int tick = 0;tick<=3;i++) {
				if(ticked[i][tick]) {
					toBuild+=curQuestion.getAnswers().get(i).getKey()+",";
				}
			}
